...
function findAllPSiblings(where) {
  let resArr = [],
    length = where.children.length;

  for (let i = 0; i < length; i++) {
    let currentElem = where.children[i],
      nextElem = where.children[i].nextElementSibling;

    if (nextElem && nextElem.tagName === "P") {
      resArr.push(currentElem);
    }
  }
...
function deleteTextNodes(where) {
  for (let i = 0; i < where.childNodes.length; i++) {
    let currentElem = where.childNodes[i];

    if (currentElem.nodeType === 3) {
      where.removeChild(currentElem);
    }
  }
}
